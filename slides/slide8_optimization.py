...
class OptimizationSlide(Slide):
    def construct(self):
        ...
        # 4. Speed Optimization (Text + Visual)
        bullet3 = Text(
        ...
        # 5. Evaluation (Text + Visual)
        bullet4 = Text(
            "• Evaluation: Incorporated LPIPS (Learned Perceptual\nImage Patch Similarity) metrics for validation.", 
            font_size=bullet_font, line_spacing=1.2
        ).next_to(bullet3, DOWN, buff=0.5).align_to(bullet1, LEFT)

        # Draw a quick chart to represent metrics
        axes = Axes(
            x_range=[0, 5, 1], y_range=[0, 5, 1], 
            x_length=2, y_length=1.5,
            axis_config={"color": LIGHT_GREY}
        ).shift(RIGHT * 5.5 + DOWN * 1.5)
        graph = axes.plot(lambda x: 0.5 * x**2, color=YELLOW)
        metric_label = Text("LPIPS Score", font_size=12, color=YELLOW).next_to(axes, UP, buff=0.1)
        metric_group = VGroup(axes, graph, metric_label)

...
class MoERoutingSlide(Slide):
    def construct(self):
        ...
        # 2. The Router
        router = Circle(radius=0.8, color=YELLOW)
        router_text = Text("Router\n(SNR)", font_size=20).move_to(router.get_center())
        router_group = VGroup(router, router_text).shift(LEFT * 4)

        # 3. High-Noise Expert
        expert_high = Rectangle(width=3.5, height=1.5, color=ORANGE).shift(RIGHT * 2 + UP * 1.5)
        ...
        # 4. Low-Noise Expert
        expert_low = Rectangle(width=3.5, height=1.5, color=TEAL).shift(RIGHT * 2 + DOWN * 1.5)
        ...
        # 5. Arrows from router to experts
        ...
        # 7. Clean up High-Noise token, bring in Low-Noise token
